...
int main() {
  ...
  while (escolha != 'j') {
    ...
    switch (escolha) {
      case 'a': {
        grafo.limparGrafo();
        FILE *arquivo = fopen("grafoteste.txt", "rt");
    ...
    return 0;
      ...
      case 'b': {
        grafo.imprimirGrafoParaArquivo("resposta.txt", tipoGrafo); // Passe o tipoGrafo como argumento
    break;
    ...
    std::getline(std::cin, nome); // Use getline para ler o nome completo com espaços
