...
public:
  ...
  bool C3();
  void conectividade();
  void dfs(int v, std::vector<std::vector<int>>& grafo, 
  std::vector<bool>& visitado, std::stack<int>& pilha);
  void dfsTransposto(int v, std::vector<std::vector<int>>& grafo_reverso, 
  std::vector<bool>& visitado, std::vector<int>& componente);
...
void TGrafo::dfs(int v, std::vector<std::vector<int>>& grafo, std::vector<bool>& visitado, std::stack<int>& pilha) {
    visitado[v] = true;
    for (int u : grafo[v]) {
        if (!visitado[u]) {
...
void TGrafo::dfsTransposto(int v, std::vector<std::vector<int>>& grafo_reverso, std::vector<bool>& visitado, std::vector<int>& componente) {
    visitado[v] = true;
    componente.push_back(v);
    for (int u : grafo_reverso[v]) {
        if (!visitado[u]) {
            dfsTransposto(u, grafo_reverso, visitado, componente);
        }
    }
}
